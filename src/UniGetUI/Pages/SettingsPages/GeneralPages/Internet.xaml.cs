...
namespace UniGetUI.Pages.SettingsPages.GeneralPages
{
    ...
    public sealed partial class Internet : Page, ISettingsPage
    {
        public Internet()
        {
            ...
            Brush SUCCESS_BG = (Brush)
                Application.Current.Resources["SystemFillColorSuccessBackgroundBrush"];
            Brush WARN_BG = (Brush)
                Application.Current.Resources["SystemFillColorCautionBackgroundBrush"];
            Brush ERROR_BG = (Brush)
            ...
            foreach (var manager in PEInterface.Managers)
            {
                ...
                ProxyPanel.Children.Add(
                    new Border()
                    {
                        CornerRadius = new CornerRadius(4),
                        Padding = new Thickness(2),
                        Background =
                            level is ProxySupport.No
                                ? ERROR_BG
                                : (level is ProxySupport.Partially ? WARN_BG : SUCCESS_BG),
                        ...
                        Child = new TextBlock()
                        {
                            Text = (
                                level is ProxySupport.No
                                    ? no
                                    : (level is ProxySupport.Partially ? part : yes)
                            ),
                            TextAlignment = TextAlignment.Center,
                            Foreground = TEXT_FG,
                        },
        ...
        private async Task _usernameBox_TextChanged()
        {
            ...
            if (oldusername != UsernameBox.Text)
                return;
